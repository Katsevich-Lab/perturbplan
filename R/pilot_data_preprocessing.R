...
#' Obtain Gene ID ↔ Gene Name Table from Cell Ranger Output
#'
#' `obtain_gene_id()` reads a `molecule_info.h5` file generated by
#' **Cell Ranger** (10x Genomics) and returns a tidy mapping between
#' Ensembl gene identifiers and gene symbols. The function extracts the
#' data stored in the HDF5 datasets `features/id` and `features/name`
#' (or alternative paths supplied via the arguments) and combines them
#' into a two-column `data.frame`.
#'
#' @param path_to_cellranger_output Character. Path to an SRR-named folder
#'   (e.g. \code{"SRR12345678"}).
#' @param id_path A string. Path within the HDF5 file to the gene IDs
#'   dataset. Default is `"features/id"`.
#' @param name_path A string. Path within the HDF5 file to the gene names
#'   dataset. Default is `"features/name"`.
#'
#' @return A `data.frame` with the columns:
#' \itemize{
#'   \item \code{gene_id}: Ensembl gene ID.
#'   \item \code{gene_name}: Gene symbol.
#' }
#'
#' @details The function relies on the \pkg{rhdf5} package for HDF5 I/O.
#'   If the package is not installed, the function will throw an error.
#'
#'
#' @importFrom rhdf5 h5read
#' @export
obtain_gene_id <- function(path_to_cellranger_output,
                           id_path = "features/id",
                           name_path = "features/name") {
  if (!requireNamespace("rhdf5", quietly = TRUE)) {
    stop("Package 'rhdf5' is required but not installed.", call. = FALSE)
  }

  # Read gene IDs and gene names
  gene_id <- rhdf5::h5read(file.path(path_to_cellranger_output, "outs", "molecule_info.h5"), id_path)
  gene_name <- rhdf5::h5read(file.path(path_to_cellranger_output, "outs", "molecule_info.h5"), name_path)
