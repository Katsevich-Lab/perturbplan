...
#' calculates an average library size (if not provided) and uses it to estimate baseline
#' expression levels, after which it calls \code{\link{compute_power_posthoc}} to get
#' power estimates and expected number of discoveries.
#'
#' @param library_size (Optional) A numeric value specifying the average library size (i.e.,
#'   average UMI count per cell). If this is \code{NULL}, the function attempts to compute
#'   it internally using \code{num_planned_cells}, \code{recovery_rate}, \code{num_total_reads},
#'   \code{mapping_efficiency}, \code{umi_per_cell}, and \code{umi_variation}.
...
#'   generated by sequencing. This is used in estimating \code{library_size} if not provided.
#' @param mapping_efficiency A numeric value (between 0 and 1) indicating the
#'   fraction of reads that successfully map to the transcriptome. This is used 
#'   in estimating \code{library_size} if not provided.
#' @param cells_per_grna A data frame specifying how many cells contain each gRNA, with
#'   columns \code{grna_id}, \code{grna_target}, and \code{num_cells}.
...
#'   for all gRNA-gene pairs (or a data frame with \code{grna_target}, \code{response_id}
#'   columns if per-pair values are desired).
...
#'   \code{response_id} columns if per-pair values are desired).
#' @param num_planned_cells (Optional) A numeric value indicating the total planned number
#'   of cells before losses in library preparation. Used to compute \code{library_size} if
#'   \code{library_size} is \code{NULL}.
#' @param control_group A character string specifying the control group, either
#'   \code{"complement"} or \code{"nt_cells"}. This is passed along to
#'   \code{compute_power_posthoc}.
#' @param umi_per_cell A numeric value specifying the theoretical saturation level (in UMIs)
#'   for each cell. This is used in estimating \code{library_size} if not provided.
#' @param umi_variation A numeric value controlling how overdispersion in UMIs per read
#'   is modeled. This is used in estimating \code{library_size} if not provided.
...
#'   testing correction method to use, either \code{"BH"} or \code{"bonferroni"}. Defaults
#'   to \code{"BH"}.
...
#' @param cutoff (Optional) A numeric value between 0 and 1 to use as the p-value cutoff.
#'   If \code{NULL}, the function will determine it automatically using the specified
...
power_function <- function(
    ...
    umi_per_cell=NULL, umi_variation=NULL, library_size=NULL,
  ...
  if(is.null(library_size)){
    ...
    avg_library_size <- umi_per_cell * (1 - exp(-reads_per_cell / umi_per_cell) * (1 + umi_variation * reads_per_cell^2 / (2*umi_per_cell^2)))
  }else{
    avg_library_size <- library_size
  }
