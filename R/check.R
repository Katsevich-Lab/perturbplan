...
input_check_power_function <- function(
  ...
  ############################## library_size vs. other parameters #######################
  if (is.null(library_size)) {
    # library_size is NULL => must provide all parameters needed to compute library_size
    if (is.null(num_total_reads) || !is.numeric(num_total_reads) || num_total_reads <= 0) {
      stop("Since `library_size` is NULL, `num_total_reads` must be a positive numeric value!")
    }
    if (is.null(mapping_efficiency) || !is.numeric(mapping_efficiency) || 
        mapping_efficiency <= 0 || mapping_efficiency > 1) {
      stop("Since `library_size` is NULL, `mapping_efficiency` must be a numeric in (0,1]!")
    }
    if (is.null(umi_per_cell) || !is.numeric(umi_per_cell) || umi_per_cell <= 0) {
      stop("Since `library_size` is NULL, `umi_per_cell` must be a positive numeric value!")
    }
    if (is.null(umi_variation) || !is.numeric(umi_variation) || umi_variation < 0 || umi_variation > 1) {
      stop("Since `library_size` is NULL, `umi_variation` must be a numeric in [0,1]!")
    }
  } else {
    # library_size is NOT NULL => check that it is valid and
    # optionally relax or enforce that others are NULL
    if (!is.numeric(library_size) || library_size <= 0) {
      stop("If `library_size` is not NULL, it must be a positive numeric value!")
    }
    # Optionally enforce the other parameters be NULL to avoid ambiguity:
    # if (!is.null(num_total_reads) || !is.null(mapping_efficiency) ||
    #     !is.null(umi_per_cell) || !is.null(umi_variation)) {
    #   stop("If `library_size` is provided, please set `num_total_reads`, ",
    #        "`mapping_efficiency`, `umi_per_cell`, and `umi_variation` to NULL.")
    # }
  }
  ...
  if (is.null(baseline_relative_expression_stats) || 
      !is.data.frame(baseline_relative_expression_stats)) {
  ...
  if (!is.null(multiple_testing_alpha) &&
      (!is.numeric(multiple_testing_alpha) || multiple_testing_alpha <= 0 || multiple_testing_alpha >= 1)) {
  ...
  ############################## QC Thresholds ###########################################
  if (!is.numeric(n_nonzero_trt_thresh) || length(n_nonzero_trt_thresh) != 1 ||
      n_nonzero_trt_thresh < 0 || n_nonzero_trt_thresh %% 1 != 0) {
    stop("`n_nonzero_trt_thresh` must be a nonnegative integer!")
  }
  if (!is.numeric(n_nonzero_cntrl_thresh) || length(n_nonzero_cntrl_thresh) != 1 ||
      n_nonzero_cntrl_thresh < 0 || n_nonzero_cntrl_thresh %% 1 != 0) {
    stop("`n_nonzero_cntrl_thresh` must be a nonnegative integer!")
  }
