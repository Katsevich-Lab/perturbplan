...
# Generate saturation curve using preseqR formula
reads_range <- seq(1000, 100000, length.out = 200)
umis_predicted <- UMI_max * (1 - (1 + variation * reads_range / UMI_max)^(-1/variation))
...
ggplot(saturation_df, aes(x = Reads_per_Cell, y = UMIs_per_Cell)) +
  ...
  ), aes(x = Reads_per_Cell, y = UMI_max * (1 - (1 + variation * Reads_per_Cell / UMI_max)^(-1/variation))),
  size = 3, color = "darkgreen") +
  geom_text(data = data.frame(
    Reads_per_Cell = c(20000, 40000, 80000),
    Design = c("Wide", "Balanced", "Deep"),
    UMI = UMI_max * (1 - (1 + variation * c(20000, 40000, 80000) / UMI_max)^(-1/variation))
