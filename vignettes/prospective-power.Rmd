...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "figures/prospective-power-",
  out.width = "100%",
  fig.width = 7,
  fig.height = 5
)
...
UMI_max <- library_params$UMI_per_cell_at_saturation

# Generate saturation curve using fit_read_UMI_curve_cpp
reads_range <- seq(1000, 100000, length.out = 200)
umis_predicted <- fit_read_UMI_curve_cpp(reads_range, library_params)
...
ggplot(saturation_df, aes(x = Reads_per_Cell, y = UMIs_per_Cell)) +
  ...
  ), aes(x = Reads_per_Cell, y = fit_read_UMI_curve_cpp(c(20000, 40000, 80000), library_params)),
  size = 3, color = "darkgreen") +
  geom_text(data = data.frame(
    Reads_per_Cell = c(20000, 40000, 80000),
    Design = c("Wide", "Balanced", "Deep"),
    UMI = fit_read_UMI_curve_cpp(c(20000, 40000, 80000), library_params)
  ...
  labs(
    ...
    subtitle = sprintf("UMI_per_cell_at_saturation = %d, method = %s",
                      round(UMI_max), library_params$method_used),
    x = "Reads per Cell",
    y = "UMIs per Cell",
